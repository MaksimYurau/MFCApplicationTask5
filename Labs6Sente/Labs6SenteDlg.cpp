...
void CLabs6SenteDlg::OnClickedExtractSelected()
{
	...
	int count = pListBox->GetSelCount();

	// Выделить память для массива, содержащего индексы выделенных элементов
	CArray<int, int> arrSelected;
	arrSelected.SetSize(count);

	// Заполнить массив индексами выделенных элементов
	pListBox->GetSelItems(count, arrSelected.GetData());

	// Очистить текст в редакторе
	pEditCell->SetWindowText(_T(""));

	// Перебрать выделенные элементы и добавить их значения в редактор
	for (int i = 0; i < count; i++)
	{
		CString strItem;
		pListBox->GetText(arrSelected[i], strItem);
		CString strEdit;
		pEditCell->GetWindowText(strEdit);
		strEdit += (strItem + _T(" "));
		pEditCell->SetWindowText(strEdit);
	}
